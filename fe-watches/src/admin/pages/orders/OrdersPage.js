...
const OrdersPage = () => {
  ...
  // Advanced filter states (giữ lại vì phức tạp)
  ...
  // Orders state (cần thiết cho table)
  const [orders, setOrders] = useState([]);
  const [total, setTotal] = useState(0);

  // Custom hooks
  const { hasAccess } = useAccessControl();
  const { currentPage, totalPages, setCurrentPage, setTotalPages, pageSize, setPageSize } = usePagination(10);
  const { searchText, setSearchText, debouncedSearchText } = useSearchAndFilter();
  const { loading, get, post, put, del } = useApiCall();
  const { createData, updateData, deleteData } = useCRUD({
    baseUrl: ORDER_ENDPOINTS.ORDERS,
    entityName: 'đơn hàng'
  });
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const result = await get(ORDER_ENDPOINTS.ORDERS, paramsObj, 'Lỗi khi tải danh sách đơn hàng');
  ...
  }, [debouncedSearchText, filterType, dateRange, statusFilter, paymentMethodFilter, paymentStatusFilter, shippingMethodFilter, isOnlineOrderFilter, totalAmountMin, totalAmountMax, storeFilter, employeeFilter, currentPage, pageSize, hasAccess, get, setTotalPages]);

  const searchCustomers = async (searchText) => {
    if (!hasAccess) return;
    
    setCustomerSearchLoading(true);
    const result = await get(CUSTOMER_ENDPOINTS.CUSTOMERS, { search: searchText }, 'Lỗi khi tìm kiếm khách hàng');
    
    if (result.success && result.data) {
      setCustomers(Array.isArray(result.data.results) ? result.data.results : []);
    }
    setCustomerSearchLoading(false);
  };

  // Debounce search text for customers
  useEffect(() => {
  ...
  const fetchEmployees = async () => {
    ...
    const result = await get(STORE_ENDPOINTS.EMPLOYEES_LIST_ALL, {}, 'Lỗi khi tải danh sách nhân viên');
  ...
  const fetchProducts = async () => {
    ...
    const result = await get(PRODUCT_ENDPOINTS.PRODUCTS, {}, 'Lỗi khi tải danh sách sản phẩm');
  ...
  const fetchVariants = async (productId) => {
    ...
    const result = await get(PRODUCT_ENDPOINTS.PRODUCT_VARIANTS(productId), {}, 'Lỗi khi tải danh sách biến thể');
  ...
  const fetchCoupons = async () => {
    ...
    const result = await get(ORDER_ENDPOINTS.COUPONS, {}, 'Lỗi khi tải danh sách mã giảm giá');
  ...
  useEffect(() => {
    ...
    fetchAll();
  }, [debouncedSearchText, hasAccess, currentPage, fetchOrders]);

  // Reset page when filters change (hook sẽ tự động reset khi search thay đổi)
  useEffect(() => {
    setCurrentPage(1);
  }, [filterType, dateRange, statusFilter, paymentMethodFilter, paymentStatusFilter, shippingMethodFilter, isOnlineOrderFilter, totalAmountMin, totalAmountMax, storeFilter, employeeFilter]);
